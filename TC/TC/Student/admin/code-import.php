    ...
    if(in_array($file_ext, $allowed_ext))
    {
        ...
        foreach($data as $row)
        {
            if($count > 0)
            {
                ...
                $gender=$row[7];
                if (is_numeric($row[8])) {
                    // Convert Excel date to PHP date object
                    $dob = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[8])->format('Y-m-d');
                } else {
                    // Convert date string to PHP date object
                    $dob = date('Y-m-d', strtotime($row[8]));
                }
                $branch=$row[9];
                $deg=$row[10];
                $conn = mysqli_connect("localhost", "root", "1711104@gceb!!", "db_admission");

                // Check for duplicate entries
                $checkQuery = "SELECT * FROM student WHERE id='$id' and regno='$regno'";
