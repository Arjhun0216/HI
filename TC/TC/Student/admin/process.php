...
if (isset($_POST['download'])) {
    if (!empty($_POST['regno'])) {
        // Get selected regno values
        $regnos = $_POST['regno'];
        // Create a comma-separated list of regnos
        $regnosList = implode(',', $regnos);

        // Redirect to demo.php with the regno values
        header("Location: demo.php?regnos=" . urlencode($regnosList));
        exit;
    } else {
        echo "No regno selected.";
        // You can redirect back to the form or handle it as needed
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['regno'])) {
    ...
    foreach ($selectedRegnos as $regno) {
        ...
        $stmtInsertTclist = $conn->prepare("INSERT INTO tclist (regno) VALUES (?)");
        ...
        $stmtInsertTclist->bind_param("s", $regno);

        // Execute the statement and check for success
        if (!$stmtInsertTclist->execute()) {
            echo "Error inserting regno '$regno' into tclist table: " . $stmtInsertTclist->error . "<br>";
        } else {
            echo "Successfully inserted regno '$regno' into tclist table.<br>";
        }

        // Close the statement after execution
        $stmtInsertTclist->close();
        ...
        if ($result->num_rows > 0) {
            ...
            $pdf->SetFont('pala','','12');
            $pdf->SetXY(175,40);
            $pdf->Cell(10,20,$row['tcno'].'/'.$row['tcyear'],0,1,'L',false);

            $data2=array(
                array(
                    "1", "Name of the  Institution", $row['college']
                ),
                array(
                    "2", "Name of the Student", $row['sname']
                ),
                array(
                    "3", "Father (or) Mother/Gurdian Name", $row['gname']
                ),
                array(
                    "4", "Nationality & Religion", $row['nation'].'-'.$row['religion']
                ),
                array(
                    "5", "Community&Caste", $row['community']
                ),
                array(
                    "6", "Gender", $row['gender']
                ),
                array(
                    "7", "Date of Birth", formattedDate($row['dob'])
                ),
                array(
                    "8", "Class in which the student was studying at the time of leaving Year/Semester", $row['lastyear']
                ),
                array(
                    "9", "Branch", $row['grad'].'-'.$row['division']
                ),
                array(
                    "10", "Date of the Admission", formattedDate($row['doa'])
                ),
                array(
                    "11", "Whether qualified for promotion to higher class", $row['highclass']
                ),
                array(
                    "12", "Whether the student has paid all the fees due to the Institution", $row['fees']
                ),
                array(
                    "13", "Whether the Student was in receiver of any Scholarship or any educational concessions", $row['scholarship']
                ),
                array(
                    "14", "Whether the Student has undergone medical inspection during the year [Fisrt or Repeat to be specified]", $row['medical']
                ),
                array(
                    "15", "Medium of Instruction", $row['medium']
                ),
                array(
                    "16", "Date on Which the Student actually left the Institution", formattedDate($row['lid'])
                ),
                array(
                    "17", "Date on which the student applied for the transfer Certificate", formattedDate($row['tcapply'])
                ),
                array(
                    "18", "Date of transfer Certificate", formattedDate($row['tcd'])
                ),
                array(
                    "19", "Academic year", $row['acdyear']
                ),
                array(
                    "20", "Conduct and Character of the Student", $row['conduct']
                )
            );
